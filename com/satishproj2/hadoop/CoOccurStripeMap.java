...
public class CoOccurStripeMap extends Mapper<LongWritable, Text, Text, MapWritable> {

	private final static IntWritable one = new IntWritable(1);
	private Text word = new Text();
	 private MapWritable occurrenceMapStripe = new MapWritable();
	...
	protected void map(LongWritable key, Text value,
		...
		while (tokenizer.hasMoreTokens()) {
			...
			while(stInnterTokenizer.hasMoreTokens())
			{
				...
				{
					if(occurrenceMapStripe.containsKey(innerLoopWord))
					{
						IntWritable counter = (IntWritable)occurrenceMapStripe.get(innerLoopWord);
						counter.set(counter.get()+1);
					}else{
						occurrenceMapStripe.put(new Text(innerLoopWord),new IntWritable(1));
					}
				}
				 context.write(new Text(outerLoopWord),occurrenceMapStripe);
